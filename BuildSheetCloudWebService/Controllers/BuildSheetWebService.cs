            ...
            try
            {
                ...
                try
                {
                    ...
                    if (selectIndex == -1 || fromIndex == -1 || fromIndex <= selectIndex)
                    {
                        ...
                        return BadRequest(new { message = "Invalid query format. Please include a valid SELECT, FROM and WHERE clause." });
                    }
        ...
        public async Task<IActionResult> UpdateQuery([FromQuery] string query, [FromQuery] string rowVersion)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        await transaction.RollbackAsync();
                        return BadRequest(new {message = "Invalid query format. Please include a valid SET and WHERE clause."});
                    }
                    ...
                    {
                        await transaction.RollbackAsync();
                        return BadRequest(new { message = "Conflict found. Please redo." });
                    }
                }
                catch (Exception)
                {
